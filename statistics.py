...
def get_predictions(X_list, y_list, idx_list, model, num_functions, n_features, max_length):
    ...
    for i in range(0,len(idx_list)): 
        ...
        #append to all lists 
        all_prob_list.append(softmax)  #needs to be from one onwards otherwise includes probability of being predicted to be unknown 
...
def calculate_metrics(all_cat_list, all_prob_list, thresholds, categories, num_functions): 
    ...
    for i in range(len(all_prob_list)):
        ...
        if np.max(all_prob_list[i]) > thresh_look.get(np.argmax(all_prob_list[i])):
            ...
            if all_cat_list[i] == np.argmax(all_prob_list[i]):
                ...
                false_positive[np.argmax(all_prob_list[i])-1] += 1 #category predicted is a false positive 
                false_negative[all_cat_list[i] -1] += 1 #category which was meant to be predicted is a false negative 

        else:

            false_negative[all_cat_list[i] - 1] += 1 #actual category which is below the required threshold

    precision = true_positive/(true_positive + false_positive)
    recall = true_positive/(true_positive + false_negative)
    f1 = 2*precision*recall/(precision + recall)
    
    ave_precision = np.sum(true_positive)/(np.sum(true_positive) + np.sum(false_positive))
    ave_recall = np.sum(true_positive)/(np.sum(true_positive) + np.sum(false_negative))
    ...
    return {'category': category_list, 'precision': precision_list, 'recall': recall_list, 'f1': f1_list}
